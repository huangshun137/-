    ...
    <el-col :span="24" class="warp-main" v-loading="loading" element-loading-text="拼命加载中">
      ...
      <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
        <el-form :inline="true" :model="filters">
          <el-form-item>
            <el-input v-model="filters.name" placeholder="新闻" @keyup.enter.native="handleSearch"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <!--列表-->
      <el-table :data="news" highlight-current-row @selection-change="selsChange" style="width: 100%;">
      ...
      <el-dialog title="编辑" :visible.sync ="editFormVisible" :close-on-click-modal="false">
        <el-form :model="editForm" label-width="100px" :rules="editFormRules" ref="editForm">
          ...
          <el-form-item label="日期">
            <el-date-picker :picker-options="startDatePicker" type="date" placeholder="选择日期(默认当前日期)" v-model="editForm.publishAt"></el-date-picker>
          </el-form-item>
...
<script>
  ...
  export default{
    data(){
      return {
        startDatePicker: this.startTime(),
        filters: {
          name: ''
        },
        news: [],
    ...
    methods: {
      ...
      search(){
        ...
        that.loading = true;
        API.getNewsList(params).then(function (result) {
          that.loading = false;
          if (result) {
            that.total = result.total;
            for (var i in result.news) {
              // console.log(result.news[i].time.toLocaleString())
              // result.news[i].time = result.news[i].time.toLoaclString();
              result.news[i].time = that.dateFilter(result.news[i].time)
            }
...
<style>
  ...
  td .el-form--inline .el-form-item__label{
    display: block;
  }
</style>
